...
class L:
 def __init__(s, r):
  ...
  s.mf.grid_columnconfigure(0, weight=0); s.mf.grid_columnconfigure(1, weight=1); s.mf.grid_rowconfigure(0, weight=1)
  s.load_session(); s.sm()
 def cf(s):
 ...
 def load_session(s):
  if os.path.exists("session.dat"):
   try:
    with open("session.dat", "r", encoding="utf-8") as f: s.parse_data(f.read().strip(), silent=True)
   except: pass
 def parse_data(s, url, silent=False):
  try:
   ...
   if "nial_hash" in q:
    ...
    with open("session.dat", "w", encoding="utf-8") as f: f.write(url)
    if not silent: mb.showinfo("NIAL API", f"Авторизация успешна!\nДобро пожаловать, {s.user}!")
   elif "nial_token" in q:
    s.user = f"TOKEN: {q['nial_token'][0].upper()}"; s.av_colors = None
    with open("session.dat", "w", encoding="utf-8") as f: f.write(url)
  except Exception as e:
   if not silent: mb.showerror("NIAL API Error", f"Критический сбой дешифрации пакета: {e}")
 def sb(s):
 ...
 def sm(s):
  ...
  log = ["• [ФИЗИКА] Исправлен баг со спавном пиплов на ЛКМ.", "• [ПАСХАЛКА] Добавлен динамический свет факела и красные глаза в ПЕЩЕРЕ.", "• [МОДЫ] Добавлен продвинутый менеджер модов с окном переключения статусов.", "• [СОХРАНЕНИЕ] Исправлено автосохранение профиля: сессия теперь вечно кэшируется в файл."]
 ...
 def sw_p(s):
  ...
  def enter_nial_url():
   ...
   if url: s.parse_data(url); s.sw_p()
  ...
  btn_reg = tk.Button(s.rf, text="Впервые на NIAL? Создайте бесплатную учетную запись", font=("Arial", 11, "underline"), fg="#b4b4be", bg="#0f0f12", bd=0, activebackground="#0f0f12", activeforeground="#ffffff", command=lambda: wb.open("https://nialcraft-official.github.io/NialCraftAccounts.github.io/#register"))
 ...
 def sw_a(s):
  ...
  btn_git = tk.Button(s.rf, text="ОТКРЫТЬ РЕПОЗИТОРИЙ НА GITHUB", font=("Arial", 12, "bold"), fg="white", bg="#235a23", width=32, height=2, bd=0, command=lambda: wb.open("https://github.com/NialCraft-official/NialCraftAccounts.github.io/blob/main/README.md"))
